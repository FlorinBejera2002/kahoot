...
export default function PlayerCard({ player, showScore = false, showRank = false, index = 0 }) {
  ...
  return (
    <div
      className="flex items-center gap-3 bg-white border border-gray-200 rounded-lg px-4 py-3 hover:shadow-sm transition-all animate-slide-up"
      style={{ animationDelay: `${index * 80}ms` }}
    >
      {showRank && rank && (
        <span className={`font-bold text-lg w-8 text-center ${RANK_COLORS[rank] || 'text-gray-500'}`}>
          #{rank}
        </span>
      )}
      <Avatar src={avatar_url} name={nickname} size="sm" />
      <div className="flex-1 min-w-0">
        <p className="font-semibold truncate text-gray-900">{nickname}</p>
        {streak > 1 && (
      ...
      {showScore && (
        <div className="text-right flex items-center gap-2">
          <span className="font-bold text-lg text-gray-900">{formatScore(total_score)}</span>
