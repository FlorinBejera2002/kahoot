  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-gray-50">
      <div className="text-center animate-fade-in">
        ...
        <h1 className="text-3xl font-bold font-display mb-2 text-gray-900">You're in!</h1>

        <div className="flex flex-col items-center gap-3 my-6">
          <Avatar src={playerSession.avatar_url} name={playerSession.nickname} size="md" />
          <p className="text-xl font-semibold text-gray-900">{playerSession.nickname}</p>
        </div>

        <p className="text-gray-500 mb-2">Game PIN: <span className="font-bold text-gray-900">{pin}</span></p>
        ...
        <p className="text-gray-400 mt-4">Waiting for host to start...</p>

        {players.length > 0 && (
          <div className="mt-8">
            <p className="text-gray-400 text-sm mb-3">{players.length} player{players.length !== 1 ? 's' : ''} in lobby</p>
            <div className="flex flex-wrap justify-center gap-2">
              {players.filter((p) => p.is_active !== false).map((p) => (
                <div key={p.id} className="flex items-center gap-2 bg-white border border-gray-200 rounded-full px-3 py-1 shadow-sm">
